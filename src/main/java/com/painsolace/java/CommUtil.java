...
import com.painsolace.java.common.StringUtils;
...
public class CommUtil {
	 ...
	 * 发送短信验证码
	 */
	public static String sendSMS(String phone, String msgText) throws Exception {
		String url = "http://222.73.117.158/msg/";// 应用地址
		String account = "qdleyin";// 账号
		String pswd = "QDleyin123";// 密码
		String mobile = phone;// 手机号码，多个号码使用","分割
		String msg = msgText;// 短信内容
		boolean needstatus = true;// 是否需要状态报告，需要true，不需要false
		String product = null;// 产品ID
		String extno = null;// 扩展码
		String returnString = null;
		System.out.println(phone + " " + msgText);
		...
		if(result == HttpStatus.SC_OK){
			...
			returnString = URLDecoder.decode(baos.toString(),"UTF-8");
		} else {
		...
		if (pages > 0) {
			...
			if (beginPage <= pages) {
				...
				for (int j = 0; (i <= pages) && (j < 5); j++) {
					if (i == currentPage) {
						s.append("<a class='this' href='").append(url).append("_").append(i).append(".htm'>").append(i)
								.append("</a> ");
					} else {
						s.append("<a href='").append(url).append("_").append(i).append(".htm'>").append(i)
								.append("</a> ");
					}
					i++;
				}
	...
	//前端分页显示用的链接  首页 上一页  3 4 5  下一页 尾页
	public static String showPageHtml(String url, String params, int currentPage, int pages) {
		...
		if (pages > 0) {
			if (currentPage >= 1) {
				...
				if (currentPage > 1) {
					s.append("<a href='" + url + "?currentPage=" + (currentPage - 1) + params + "'>上一页</a> ");
				}
			}
			int beginPage = currentPage - 3 < 1 ? 1 : currentPage - 3;
			if (beginPage <= pages) {
				...
				for (int j = 0; (i <= pages) && (j < 5); j++) {
					if (i == currentPage) {
						s.append("<a class='this' href='" + url + "?currentPage=" + i + params + "'>" + i + "</a> ");
					} else {
						s.append("<a href='" + url + "?currentPage=" + i + params + "'>" + i + "</a> ");
					}
					i++;
				}
				s.append("页　");
			...
			if (currentPage <= pages) {
				if (currentPage < pages) {
					s.append("<a href='" + url + "?currentPage=" + (currentPage + 1) + params + "'>下一页</a> ");
				}
	...
	public static String showPageFormHtml(int currentPage, int pages) {
		...
		if (pages > 0) {
			if (currentPage >= 1) {
				...
				if (currentPage > 1) {
					s.append("<li><span><a href='javascript:void(0);' onclick='return gotoPage(" + (currentPage - 1)
							+ ")'>上一页</a></span></li> ");
				}
			}
			int beginPage = currentPage - 3 < 1 ? 1 : currentPage - 3;
			if (beginPage <= pages) {
				...
				for (int j = 0; (i <= pages) && (j < 6); j++) {
					if (i == currentPage) {
						...
						s.append("<li><span><a class\"demo\" href='javascript:void(0);' onclick='return gotoPage(" + i
								+ ")'>" + i + "</a></span></li> ");
					}
					i++;
				}
			...
			if (currentPage <= pages) {
				...
				s.append("<li><span><a href='javascript:void(0);' onclick='return gotoPage(" + pages
						+ ")'>尾页</a> </span></li>");
			}
		}
		s.append("</ul>");
	...
	public static String showPageFormAtagHtml(int currentPage, int pages) {
		...
		if (pages > 0) {
			if (currentPage >= 1) {
				...
				if (currentPage > 1) {
					s.append("<a href='javascript:void(0);' onclick='return gotoPage(" + (currentPage - 1)
							+ ")'>上一页</a> ");
				}
			}
			int beginPage = currentPage - 3 < 1 ? 1 : currentPage - 3;
			if (beginPage <= pages) {
				...
				for (int j = 0; (i <= pages) && (j < 6); j++) {
					if (i == currentPage) {
						s.append("<a class='this' href='javascript:void(0);' onclick='return gotoPage(" + i + ")'>" + i
								+ "</a> ");
					} else {
						s.append("<a href='javascript:void(0);' onclick='return gotoPage(" + i + ")'>" + i + "</a> ");
					}
					i++;
				}
				s.append("页　");
			...
			if (currentPage <= pages) {
				...
				s.append("<a href='javascript:void(0);' onclick='return gotoPage(" + pages + ")'>尾页</a> ");
			}
		}
		return s.toString();
	}
	...
	public static String showPageAjaxHtml(String url, String params, int currentPage, int pages) {
		...
		if (pages > 0) {
			...
			if (currentPage >= 1) {
				s.append("<a href='javascript:void(0);' onclick='return ajaxPage(\"" + address + "\",1,this)'>首页</a> ");
			...
			if (beginPage <= pages) {
				...
				for (int j = 0; (i <= pages) && (j < 5); j++) {
					if (i == currentPage) {
						s.append("<a class='this' href='javascript:void(0);' onclick='return ajaxPage(\"" + address
								+ "\"," + i + ",this)'>" + i + "</a> ");
					} else {
						s.append("<a href='javascript:void(0);' onclick='return ajaxPage(\"" + address + "\"," + i
								+ ",this)'>" + i + "</a> ");
					}
					i++;
				}
			...
			if (currentPage <= pages) {
				...
				s.append("<a href='javascript:void(0);' onclick='return ajaxPage(\"" + address + "\"," + pages
						+ ",this)'>尾页</a> ");
			}
		}
		return s.toString();
	...
	public static void saveIPageList2ModelAndView(String url, String staticURL, String params, IPageList pList,
			ModelAndView mv) {
		System.out.println("\n" +pList.getCurrentPage()+ "url is " + url);
		if (pList != null) {
			...
			mv.addObject("gotoPageFormAtagHTML", showPageFormAtagHtml(pList.getCurrentPage(), pList.getPages()));
	...
	public static void saveIPageList2ModelAndView(String url, String staticURL, String params, IPageList pList,
		...
		if (pList != null) {
			...
			mv.addObject("rows", new Integer(pList.getRowCount()));
			mv.addObject("currentPage", new Integer(pList.getCurrentPage()));
			mv.addObject("gotoPageHTML", showPageHtml(url, params, pList.getCurrentPage(), pages));
			mv.addObject("gotoPageFormHTML", showPageFormHtml(pList.getCurrentPage(), pList.getPages()));
			mv.addObject("gotoPageFormAtagHTML", showPageFormAtagHtml(pList.getCurrentPage(), pList.getPages()));
			mv.addObject("gotoPageStaticHTML", showPageStaticHtml(staticURL, pList.getCurrentPage(), pList.getPages()));
			mv.addObject("gotoPageAjaxHTML", showPageAjaxHtml(url, params, pList.getCurrentPage(), pList.getPages()));
	...
	public static long mul_int(Object a, Object b) {
		double ret = mul(a, b);
		return (long) Math.floor(ret / 100);
	}
